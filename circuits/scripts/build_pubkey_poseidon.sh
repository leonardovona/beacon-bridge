...
PHASE1=../../resources/powersOfTau28_hez_final_20.ptau
BUILD_DIR=../../build
CIRCUIT_NAME=pubkey_poseidon
TEST_DIR=../../data
...
run() {
    ...
    start=`date +%s`
    ./"$OUTPUT_DIR"/"$CIRCUIT_NAME" "$TEST_DIR"/input_sync_committee_poseidon.json "$OUTPUT_DIR"/witness.wtns
    end=`date +%s`
    echo "DONE ($((end-start))s)"
